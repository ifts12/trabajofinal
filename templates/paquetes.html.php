...
$c = new Conexion();
try {
    $statement = $c->prepare('SELECT v.*, p.nombre AS provincia FROM viaje v LEFT JOIN provincia p ON v.id_provincia=p.id LEFT JOIN tipo t ON v.id_tipo=t.id WHERE t.nombre = "Paquete"');
    $statement->execute();
